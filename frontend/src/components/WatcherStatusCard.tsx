import { Antenna } from 'lucide-react';
import type { ProcessInfo } from '../lib/api';

interface Props {
  watcher: ProcessInfo;
}

export function WatcherStatusCard({ watcher }: Props) {
  const alive = watcher.alive;
  return (
    <div className="glass-strong rounded-lg p-4 flex items-center gap-3">
      <div className={`w-9 h-9 rounded-md flex items-center justify-center ${
        alive ? 'bg-emerald-500/15 text-emerald-300' : 'bg-rose-500/15 text-rose-300'
      }`}>
        <Antenna className="w-4 h-4" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-xs text-slate-500">watcher</div>
        <div className="text-sm text-slate-100">
          {alive ? '运行中' : '未运行'}
          {alive && watcher.pid && (
            <span className="ml-2 text-[11px] text-slate-500">pid {watcher.pid}</span>
          )}
        </div>
      </div>
    </div>
  );
}
