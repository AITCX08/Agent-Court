---
# agent-court workflow — agent 行为 + 运维参数, 团队 in-repo 版本管 (SY-1 #16)
#
# 改这个文件 = 改 agent 行为. 走正常 PR review 流程, 像 review 代码一样
# review prompt 演进. ~/.agent-court/projects/<id>/court.yaml 仍负责 system
# 级 (秘钥 / 端口 / federation 开关), 但 agent 行为搬到这里.
#
# Loader: mcp/court-mcp/workflow_loader.py
# 调用方: bin/spawn-issue-window (用 prompt body) + im_reply_router (用 frontmatter
# 配置, 例如 allowed_labels / max_concurrent_runs)
schema_version: 1

# Git 规则 (agent commit/push 受 pre-push hook 强制校验)
branch_prefix: "auto/issue-"

# 并发与重试 (SY-4 #17 启用 max_concurrent_runs / retry_max)
max_concurrent_runs: 3
run_timeout_seconds: 1800        # 30 min
retry_max: 3
retry_backoff_base_seconds: 60   # 60s → 120s → 240s 指数退避

# 工作目录 (SY-2 #19 启用 worktree)
working_dir_strategy: inplace    # inplace | worktree

# Issue 过滤 (空 = 不过滤; 非空 = issue label 必须命中其一)
allowed_labels: []

# 二段审批 (PR-13 dashboard 模式)
require_approval_stages:
  - INTAKE
  - PLAN

# Issue tracker (BOOT-1 #20 启用 github)
tracker:
  provider: gitea
  # base_url: ~ (留空 = 用 provider 默认; gitea→git.k2lab.ai; github→api.github.com)
---

# agent-court · issue resolver workflow

你被 agent-court dashboard 模式 spawn,负责处理**一个**指派给我的 Gitea issue 全闭环。

## 你收到的输入 (stdin)

stdin 第一段是结构化的 issue 上下文,以 `ISSUE_RESOLVER_BEGIN <repo> <num>` 开头。包含三段 JSON:
...
读 issue.json + comments.json + shenli.decision.json。shenli 已经判 GO,你**必要时**才重跑一次复核。若 issue body 在 watcher 抓到之后被人改过且偏离原意,可以拒绝(走步骤 7 拒绝路径)。
...
- `approved=false` 且 `edit_instruction` 空 → 步骤 7 拒绝路径
...
- **分支前缀必须** `auto/issue-<num>/<short-summary>`(由 `branch_prefix` frontmatter 控制,如 `auto/issue-7/add-export-button`)
- **禁止** push 到 `main`
...
- 看 hook stderr 找原因(常见: 分支不是 `branch_prefix` 起头; commit 缺 trailer)
- **不要** `--no-verify` 绕过
...
- 自动在 issue 上发完成评论(含 winner / summary)
...
- **整体卡超过 `run_timeout_seconds`(默认 30 min)无进展**: 由 retry/超时机制兜底(SY-4 #17 启用);用户也可以在 tmux window 里直接 `Ctrl-C` 中断
...
1. 不修改 `bus/` 协议,本 workflow 是独立 Claude window 流不依赖 court 多 role
2. 完成前不关 tmux window(用户可能要审 commit / push)
3. 所有 commit 必带 `Issue:` trailer(pre-push hook 校验)

## 配置变量

下面的字段由 frontmatter 决定,**不要在 prompt 文本里硬编码值**:

- `branch_prefix` → 分支前缀
- `run_timeout_seconds` → 单 issue 超时
- `allowed_labels` → 哪些 label 才接(运维过滤)
- `working_dir_strategy` → inplace 共用 repo / worktree 每 issue 独立 (SY-2)
- `tracker.provider` → gitea / github (BOOT-1)
