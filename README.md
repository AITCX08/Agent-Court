...
把你的 `agent-court` 安装想象成一座**朝廷**：

| 阶 | 技术名 | 朝官 | 干什么 |
|---|---|---|---|
| 君 | 人类（你） | **皇上 / 君王** | 颁旨；批红；最终拍板 |
| 中枢 | 上游 LLM（Claude Code / Cursor 等） | **内阁首辅 / 丞相** | 听旨，决定召唤哪一司办差 |
| 一司 | `$COURT_ROOT/projects/<p>/` 下的一个 project | **一道 / 一司**（朝廷下属的一个部门，一 project 一座） | 一个 tmux session 包着所有 role + 一间私邮间 |
| 司主事 | `foreman` 这个 role | **司主事 / 首辅大学士** | 接丞相派的活，拆成子任务交给六部主事 |
| 六部主事 | `frontend` / `backend` / `devops` / … | **六部主事**（开发岗映射到吏户礼兵刑工） | 在自己的文件里干自己的活 |
| 大理寺 | `judge.py` / LLM 裁判 | **大理寺正 / 推官** | tier_b 拿不准时由它裁决，失败 fail-safe 交人审 |
| 通政司 | 文件总线 (`bus/`) | **通政司** | 中央转呈机构，所有奏折 / 公文都在这里中转 |
| 起居官 | `court-watcher` 守护进程 | **起居官** | 起居注：记录朝廷动作 + 路由公文 |
| 鸿胪寺 | `court-mcp` MCP server | **鸿胪寺** | 掌外交：把朝廷暴露给上游 LLM 派活 |
| 留中 | `bus/<peer>/pending-approval/` | **留中** | 奏折扣在宫里、皇上还没批 |
| 批红 | `court-approve` 操作 | **朱批 / 准奏 / 驳回** | 皇上对留中奏折的最终处置 |
| 路引 / 品级令 | path / tier grant | **路引** / **品级令** | sudo 风格临时凭证：限时限路放某外藩看某文件 / 让某外藩临时升一品阶 |
| 藩邦 | `peers.yaml` 里的一个外部 court | **藩邦 / 邻邦** | 你显式联邦过、跑在别的机器上的另一个朝廷分支 |
| 邦交 | federation 块 | **邦交** | 邦国之间往来的总章程，默认关闭 |
...
agent-court MCP 调 `court-approve(...)`，文件就落到 `inbox/` 完成投递。
