...
> 一个本地的、迷你的多 Agent 编排器。你的 `agent-yamen` 安装下的每个
...
把你的 `agent-yamen` 安装当成一个小**朝廷**来想：

| 层 | 技术名 | 比喻 | 干什么 |
|---|---|---|---|
| 至上 | 人类（你） | 君王 | 下旨；审阅结果；最终拍板。 |
| 丞相 | 上游 LLM（Claude Code 等） | 丞相 / 助理 | 听君王说话，决定召唤哪个 court。 |
| 法庭 | `$YAMEN_ROOT/projects/<p>/` 下的一个 project | 府衙（一 project 一个） | 一个 tmux session 包着所有 role + 一间私邮间。 |
| 工头 | `zongguan` 这个 role | 工头 | 接丞相派的活；拆成子任务交给百官。 |
| 百官 | `frontend` / `backend` / `devops` / … | 百官 | 在自己的文件里干自己的活。 |
| 邻邦 / 上司 / 下属 court | `bangjiao.yaml` 里的一个对端 | 邻邦 | 你显式联邦过的、跑在别的机器上的另一个 `agent-yamen` project。 |

> **命名提醒**：英文里的 "peer" 是个一词两义 —— 既指"和 zongguan
> 平级的 worker"，又指"被联邦的远端 court"。在代码里我们让 `peer`
> 专指**远端 court**；同级 court 在 `bangjiao.yaml` 里用
...
`agent-yamen` 让 agent 保持可见（tmux）、持久（文件，不是内存）、
...
│          lan_chengzou                                          │
│  peer  : lie_fanbang / guoshu_fanbang    ← 签名 + POST 给远端        │
└──────────────────────────────┬───────────────────────────────────────┘
                               │ 写 markdown 文件
┌──────────────────────────────┴───────────────────────────────────────┐
...
可选的联邦（按 project，默认关）：
   ...
   ┌──────────────────────────┐               ┌──────────────────────────┐
   │ MCP: guoshu_fanbang    │ ed25519 签名   │ tongzheng :8765 /inbox  │
   │   → POST /inbox          │──────────────▶│   校验签名               │
   │                          │               │   查 expose_roles        │
   │                          │               │   落到 bus/              │
   └──────────────────────────┘               └──────────────────────────┘
...
- 一个能接 `--append-system-prompt`、可选 `--model` 的 LLM CLI。
  ...
  在你 project 的 `yamen.yaml` 里设 `default_cli`，或者在 role 级
  用 `cli` 覆盖。
...
# 改 ~/.agent-yamen/projects/myproject/yamen.yaml 里的 work_dir 路径
```
...
这会起一个名叫 `yamen-myproject` 的 tmux session，里面每个 role 一个
窗口，每个窗口跑着 LLM CLI（已经把 role 的 system prompt 加载好了）。
`qijuguan` 守护进程在后台一起起来；日志在
`~/.agent-yamen/projects/myproject/logs/`。
...
```bash
chuanwen -p myproject --to zongguan "审一下新的 auth 改动，需要的话分派给该跟进的角色"
```

zongguan 的 claude 窗口会收到一行 `[notify]`，去读 inbox 然后反应。
...
# 验证
claude mcp list   # 应该能看到 agent-yamen ✓ Connected
```
...
| 工具 | 什么时候用 |
|---|---|
| `lie_yamen` | 用户提到某个 project 名字，你想知道有哪些可选。 |
| `chizhao_zongguan(project, message, target_role?)` | 用户想让某个 court 里的人干件事。 |
| `tang_yamen(project)` | 用户问 "`<project>` 现在怎么样？" |
| `lan_chengzou(project)` | 用户问 "`<project>` 有回信吗？"（zongguan 回给上游的消息住这）。 |
| `lie_fanbang(project)` | 用户问某个 project 的联邦状态。 |
| `guoshu_fanbang(project, peer_yamen_id, message, ...)` | 用户想把一件事转发给一个被联邦的 court。 |
| `ban_luyin(project, peer_yamen_id, paths, ttl?)` | 用户想临时拓宽某个 peer 的 `attaches:` 能引用的范围。 |
| `sheng_pinji(project, peer_yamen_id, target_tier, ttl?, consume_on_use?)` | 用户想临时把某个 peer 的 tier（比如 `tier_a` → `tier_c`）提一档，限时或限一条消息。 |
| `lie_lingpai(project)` / `kan_lingpai(project, id)` / `zhui_lingpai(project, id)` | 用户想查看或撤销一个未到期的 grant。 |

本地 MCP 工具有**完整的机器访问权** —— 它们在 `$YAMEN_ROOT/projects/<p>/`
...
模型是**按 project 划分而非按机器划分**：`$YAMEN_ROOT/projects/<p>/`
下的每个 project 都有自己的密钥对、自己的 `bangjiao.yaml`、自己的
`yamen_id`。同一台机器上的两个 project，互相**无法**推断对方存在
...
# 2. 改 yamen.yaml —— 把 bangjiao: 块取消注释
$EDITOR ~/.agent-yamen/projects/myproject/yamen.yaml

# 3. 把远端 peer 加到这个 project 的 bangjiao.yaml
$EDITOR ~/.agent-yamen/projects/myproject/bangjiao.yaml
# （schema 见 projects/example/bangjiao.example.yaml）
...
当 `bangjiao: enabled: false`（或整块都没写）时，`tongzheng`
拒绝启动，`guoshu_fanbang` 返回 `{"error": "bangjiao_disabled"}`。
把 flag 翻回 false，下一个入站请求就会生效 —— 不用重启。

入站消息在落到总线之前要过四道关：

1. **签名** —— 用这个 project `bangjiao.yaml` 里对端的 `pub_key_b64`
   校验。签名不对 → 401。
2. **已知发件人** —— `from_court` 必须出现在这个 project 的
   `bangjiao.yaml` 里。未知 → 403。
3. **角色白名单** —— `to:` 这个 role 必须在
   `bangjiao.expose_roles` 里。默认是 `[zongguan]`，也就是说
   外面只能打到 zongguan，zongguan 再在内部分派。不在名单 → 403。
4. **策略引擎**（PR-2）—— 见下一小节。
...
| `denied` | `bus/<peer>/denied/` *（仅审计）* | attach 命中 deny 路径。**不会**到达 zongguan。 |
...
- **`yamen.yaml`** —
  - `bangjiao.allow_paths` / `deny_paths` —— 限制入站消息
    `attaches:` 中能引用的路径 glob。
  - `bangjiao.judge` —— tier_b 调哪个 CLI 来裁判，可选 `model`、
    可选 `prompt_file` 覆盖、`timeout_seconds`（默认 30）、
    `confidence_threshold`（默认 0.6）。`judge.cli` 没设时回落
    到顶层 `default_cli`。
- **`lvli.yaml`** —— `default_tier:`（`tier_a`/`tier_b`/`tier_c`
  之一） + 可选 `sensitive_keywords:` 列表（追加到内建之后）。

`bangjiao.yaml` 可以按 peer 钉死 `policy_tier:`；缺省时回落到
`lvli.yaml` 的 `default_tier`。
...
每个判定都会被追加到 `$YAMEN_ROOT/projects/<p>/logs/panduo.jsonl`
方便审计。
...
banling example info 4616c19a       # 完整记录 + 剩余时间 + 命中次数
...
授权是 `$YAMEN_ROOT/projects/<p>/grants/` 下的 JSON 文件，**原子写入**，
读取时严格校验（过大 / 损坏的文件会被跳过并写入 `logs/bangjiao-errors.log`
警告）。重启守护进程不会丢；`revoke` 直接删文件。从上游 LLM 看，
同一组面孔通过 `ban_luyin` / `sheng_pinji` / `kan_lingpai` /
`lie_lingpai` / `zhui_lingpai` 暴露。

每个授权入口对 `project` 参数都校验：必须是单一安全文件系统组件，
且解析后必须严格位于 `$YAMEN_ROOT/projects/` 之下。传
...
```
$YAMEN_ROOT/                                  # 默认 ~/.agent-yamen
├── projects/
│   └── myproject/
│       ├── yamen.yaml                        # project 配置（含 bangjiao 块）
│       ├── bangjiao.yaml                        # 这个 project 已知的对端
│       ├── lvli.yaml                       # PR-2: tier + 敏感词（可选）
...
│       │   ├── human/...                     # 你 CLI 发的消息落这里
│       │   └── <peer_yamen_id>/              # 入站 peer 消息，按判定分桶
...
仓库（本仓库）本身只装：
- `bin/` —— shell 脚本（`kaifu`、`bifu`、`qijuguan`、
  `chuanwen`、`shengtang`、`zhuyin`、`tongzheng`、`banling`）
- `mcp/yamen-mcp/` —— Python MCP server + peer 守护进程 + keygen
- `projects/example/` —— 一个可 fork 的示例 project（含被注释掉的
  `bangjiao:` 块作为 schema 参考）
- `docs/` —— 补充文档（LAN 部署、cc-connect 桥接 等）

你**实际**的 court 跑在 `$YAMEN_ROOT` 下（默认 `~/.agent-yamen/`），
**在仓库外**。
...
`agent-yamen` 更接近一个小操作系统：长期运行的 role 进程、文件系统
...
`shengtang` 调你的 CLI 时带上 `--append-system-prompt <prompt file>`
...
**不同的 project = 不同的 `yamen_id` + 不同的 key + 不同的 `bangjiao.yaml`**。
