"""agent-yamen — 审批 (shenpi): human-approval routing for ``human_required``
messages (PR-5).
...
2. The human reviews and acts. Approval/denial flows through one of two
   ...
   - CLI: ``pizhun <project> approve|deny <id>``
   - MCP tool: ``pizhun(project, id, action)`` (callable from any
     upstream LLM that has the agent-yamen MCP attached, including a
...
3. ``approve`` moves the file into ``bus/<peer>/inbox/`` (delivering it
   ...
   audit. Every action is appended to ``logs/shenpi-log.jsonl`` so the
...
def _safe_project_dir(project: str) -> Path:
    ...
    ``$YAMEN_ROOT/projects/``. Used by every public entry point here."""
...
def _audit_log_path(project: str) -> Path:
    return _safe_project_dir(project) / "logs" / "shenpi-log.jsonl"
...
class PendingItem:
    ...
    ----------
    project : str
        The receiving yamen's project name.
    peer : str
...
def _audit(project: str, *, action: str, item: PendingItem, by: str,
    ...
    """Append one JSON line to ``logs/shenpi-log.jsonl``. Best-effort."""
