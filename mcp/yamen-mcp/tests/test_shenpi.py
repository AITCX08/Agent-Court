...
def _seed(root: Path, project: str, *,
    ...
    fed: dict = {"enabled": True, "expose_roles": ["zongguan"]}
    if allow_paths is not None:
        fed["allow_paths"] = allow_paths
    if shenpi_block is not None:
        fed["shenpi"] = shenpi_block
    cy = {
        "project": project,
        "session": f"yamen-{project}",
    ...
    (pdir / "yamen.yaml").write_text(yaml.safe_dump(cy))
    return pdir


@pytest.fixture
def root_dir(tmp_path, monkeypatch):
    root = tmp_path / "yamen-root"
    root.mkdir()
...
def _write_pending(pdir: Path, peer: str, msg_id: str, *,
                   ...
                   msg_from: str = "upstream", msg_to: str = "zongguan") -> Path:
...
def test_approve_moves_file_to_inbox(root_dir):
    ...
    assert any(f.name.endswith("aaaa111a-upstream-to-zongguan.md")
...
def test_sweep_expired_moves_old_to_denied(root_dir):
    ...
    pa = pdir / "bus" / "bob" / "pending-approval"
    assert [f.name for f in pa.glob("*.md")][0].endswith("fffe5111-upstream-to-zongguan.md")
...
def test_audit_log_records_approval(root_dir):
    ...
    log = pdir / "logs" / "shenpi-log.jsonl"
...
def test_feishu_channel_posts_webhook(root_dir):
    ...
    assert "pizhun p approve aaaa111a" in payload["content"]["text"]
...
def test_mcp_lie_liuzhong_empty(root_dir):
    _seed(root_dir, "p")
    from server import lie_liuzhong
    out = lie_liuzhong("p")
    assert out["pending"] == []
    assert out["expired"] == []


def test_mcp_lie_liuzhong_returns_items(root_dir):
    pdir = _seed(root_dir, "p")
...
def test_mcp_pizhun_approve(root_dir):
    pdir = _seed(root_dir, "p")
    _write_pending(pdir, "bob", "aaaa111a")
    from server import pizhun
    out = pizhun("p", "aaaa111a", "approve", by="wechat-user-bob")
    assert out == {"ok": True, "result": "approved", "project": "p", "msg_id": "aaaa111a"}


def test_mcp_pizhun_invalid_action(root_dir):
    _seed(root_dir, "p")
    from server import pizhun
    out = pizhun("p", "aaaa111a", "magic")
    assert out["error"] == "invalid_action"


def test_mcp_pizhun_unknown_project():
    from server import pizhun
    out = pizhun("nonexistent-xyz", "aaaa111a", "approve")
...
def test_e2e_human_required_fires_terminal_notify(project_with_shenpi, root_dir):
    ...
    msg = _signed(identity, attaches=["bus/zongguan/inbox/x.md"])
