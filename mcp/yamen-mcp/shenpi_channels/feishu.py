"""agent-yamen — shenpi feishu (Lark) webhook channel.
...
def _format_text(item, mentions: list[str]) -> str:
    ...
    parts.append(f"approve: pizhun {item.project} approve {item.msg_id}")
    parts.append(f"deny:    pizhun {item.project} deny {item.msg_id}")
    if mentions:
