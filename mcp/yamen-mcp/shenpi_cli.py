"""agent-yamen — ``pizhun`` command-line entry point (PR-5).

Subcommands:

.. code-block:: shell

    pizhun <project> list                          # show pending items
    pizhun <project> approve <msg-id>              # release to inbox/
    pizhun <project> deny <msg-id>                 # park in denied/
    pizhun <project> cleanup                       # auto-deny anything past timeout_seconds

Lives next to the MCP server because they share a venv. The
``bin/pizhun`` shell wrapper just exec's the venv's python against this
module.
"""
...
def _cmd_list(args) -> int:
    ...
    try:
        ...
        print(f"[pizhun] {e}", file=sys.stderr)
        return 2

    pending = listing["pending"]
    expired = listing["expired"]
    if not pending and not expired:
        print(f"[pizhun] no pending items for project '{args.project}'")
        return 0
...
def _cmd_approve(args) -> int:
    ...
    if result == "approved":
        print(f"[pizhun] approved {args.msg_id} → bus/<peer>/inbox/")
        return 0
    if result == "not_found":
        print(f"[pizhun] no such pending item: {args.msg_id}", file=sys.stderr)
        return 1
    if result == "expired":
        print(
            f"[pizhun] {args.msg_id} is past timeout_seconds; run "
            f"'pizhun {args.project} deny {args.msg_id}' or 'pizhun {args.project} cleanup'.",
            file=sys.stderr,
        )
        return 1
    # io_error
    print(f"[pizhun] failed to move {args.msg_id}; check logs/shenpi-log.jsonl",
          file=sys.stderr)
    return 3
...
def _cmd_deny(args) -> int:
    ...
    if result == "denied":
        print(f"[pizhun] denied {args.msg_id} → bus/<peer>/denied/")
        return 0
    if result == "not_found":
        print(f"[pizhun] no such pending item: {args.msg_id}", file=sys.stderr)
        return 1
...
def _cmd_cleanup(args) -> int:
    ...
    if cfg is None or cfg.timeout_seconds <= 0:
        print(f"[pizhun] cleanup is a no-op when timeout_seconds is 0 or unset")
        return 0
    by = args.by or _resolve_issuer()
    try:
    ...
    if not swept:
        print(f"[pizhun] no expired items in '{args.project}'")
    else:
        for mid in swept:
            print(f"[pizhun] auto-denied {mid} (over timeout)")
    return 0
