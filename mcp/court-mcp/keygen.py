...
def main() -> int:
    ...
    if not project_dir(project).is_dir():
        print(
            f"[court-keygen] project '{project}' not found at {project_dir(project)}",
            file=sys.stderr,
        )
        print(
            "[court-keygen] create the project directory first "
    ...
    if project_priv_key_path(project).exists() and not args.force:
        ...
        print(
            f"[court-keygen] keypair already exists at {project_priv_key_path(project)}",
            file=sys.stderr,
        )
        print(
            "[court-keygen] use --force to regenerate (this invalidates trust with all peers)",
            file=sys.stderr,
        )
        print()
    ...
    print(f"[court-keygen] new keypair written for project '{project}':")
